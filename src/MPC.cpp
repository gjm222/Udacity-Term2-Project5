 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (size_t t = 0; t < N-1; t++) {
      ...
      fg[2 + psi_start + t] = psi1 - (psi0 - v0 * delta0 / Lf * dt);  //psi1 - (psi0 + v0 * delta0 / Lf * dt) is in quiz
      fg[2 + v_start + t] = v1 - (v0 + a0 * dt);
      fg[2 + cte_start + t] = cte1 - ((f0 - y0) + (v0 * CppAD::sin(epsi0) * dt));
      fg[2 + epsi_start + t] = epsi1 - ((psi0 - psides0) - v0 * delta0 / Lf * dt);  //epsi1 - ((psi0 - psides0) + v0 * delta0 / Lf * dt)  is in quiz
